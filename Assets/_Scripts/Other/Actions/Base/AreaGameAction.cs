...
public class AreaGameAction : GameAction
{
    ...
    private List<int> GetTargets(int senderEntity, ConditionAndActionArgs conditionAndActionArgs = null)
    {
        ...
        var filteredEntities = entetiesInRange.Where(a => _conditions.Select(b => b.CheckCondition(senderEntity, a, conditionAndActionArgs)).All(b => b == true)).ToList();
        //Debug.Log($"{filteredEntities.Count}");
        return filteredEntities;
    }

    public sealed override void Action(int senderEntity, int? takerEntity, ConditionAndActionArgs conditionAndActionArgs = null)
    {
        var targets = GetTargets(senderEntity, conditionAndActionArgs);
        foreach (int entity in targets)
        {
            foreach (var action in _actions)
            {
                action.Action(senderEntity, entity, conditionAndActionArgs);
            }
        }
    }
}
